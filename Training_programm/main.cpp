...
// TODO: проверить сети на скорость и точность, поменять гиперпараметры для одиночной сети, проверить функцию проверки (на проценты)
...
void test_network (T &net)
{
    ...
    // узнаём конечное число
    cursor->SeekToLast();
    std::string imgLast = cursor->key();

    cout << "Images in testing set: " << std::stoi(imgLast) << endl;
    // создаём представления ключа в бд с номером начала тестирующей выборки
    string tmpstr = std::to_string(bpo::images);
    int NumLength = 10 - tmpstr.length();
    for (int s = 0; s < NumLength; ++s)
    ...
    matrix<uchar> imgGray;
    dlib::matrix<float> train;
    std::vector<matrix<uchar>> test_samples;
    std::vector<dlib::matrix<float>> test_labels;
    std::vector<dlib::matrix<float>> predicted_labels;
    test_samples.clear();
    test_labels.clear();
    // временная точка для преобразования
    dlib::point po, lcp, rcp, lcpd, rcpd;
    ...
    while (cursor->key() != imgLast)
    {
        ...
        // перевод вектора в матрицу из флотов
        train.set_size(points.size()*2,1); //  строки и колонки
        for (int y = 0; y < points.size(); ++y)
        {
            train(y * 2) = (float)(points[y][0]);
    ...
    cout << "Done! " << test_labels.size() << " images processed in " << (boost::posix_time::microsec_clock::universal_time() - testTimemcs).total_microseconds() << " mcs." << endl;
    ...
    if (answer == 'M') // просмотр полученных изображений вручную
    {
        // окно для вывода в ручном режиме
        dlib::image_window win;
        // проверяем каждое изображение
        for (int i = 0; i < test_samples.size(); ++i)
        {
            // перевод матрицы учар в изображение
            win.clear_overlay();
            win.set_image(test_samples[i]);
            ...
            for (int j = 0; j < 31; ++j)
            {
                // расстановка правильных точек (синие)
                po.x() = test_labels[i](j*2);
                po.y() = test_labels[i](j*2+1);
                win.add_overlay(image_window::overlay_circle(po, 2, rgb_pixel(0,0,255)));
                win.add_overlay(po, po, rgb_pixel(0,0,255));
                if (j >= 19 && j <=24)
                {
                    lcpd.x() += po.x();
                    lcpd.y() += po.y();
                ...
                // расстановка угаданых точек (красные)
                po.x() = predicted_labels[i](j*2);
                po.y() = predicted_labels[i](j*2+1);
                ...
                differenceX = abs(test_labels[i](j*2) - (short)predicted_labels[i](j*2));
                differenceY = abs(test_labels[i](j*2+1) - (short)predicted_labels[i](j*2+1));

                cout << test_labels[i](j*2) << " - " << (short)predicted_labels[i](j*2) << " = " << differenceX << endl;
                cout << test_labels[i](j*2+1) << " - " << (short)predicted_labels[i](j*2+1) << " = " << differenceY << endl << endl;
            }
            ...
            //win.add_overlay(image_window::overlay_circle(lcp, 2, rgb_pixel(255,0,0)));
            win.add_overlay(lcp, lcp, rgb_pixel(255,0,0));
            //win.add_overlay(image_window::overlay_circle(rcp, 2, rgb_pixel(255,0,0)));
            win.add_overlay(rcp, rcp, rgb_pixel(255,0,0));

            //win.add_overlay(image_window::overlay_circle(lcpd, 2, rgb_pixel(0,0,255)));
            win.add_overlay(lcpd, lcpd, rgb_pixel(0,0,255));
            //win.add_overlay(image_window::overlay_circle(rcpd, 2, rgb_pixel(0,0,255)));
            win.add_overlay(rcpd, rcpd, rgb_pixel(0,0,255));

            if (abs(rcp.x() - rcpd.x()) > 3 || abs(rcp.y() - rcpd.y()) > 3) cout << "right eye incorrect" << endl;
            if (abs(lcp.x() - lcpd.x()) > 3 || abs(lcp.y() - lcpd.y()) > 3) cout << "left eye incorrect" << endl;
    ...
    if (answer == 'A') // автоматическое сравнение изображений с погрешностью
    {
        // разница в координатах
        int differenceX = 0, differenceY = 0;
        // количество правильных и неправильных (будем считать, что если точка отходит от эталона более чем на 5 пикселов, то она неправильная)
        int numright = 0, numwrong = 0;
        int numright2 = 0, numwrong2 = 0;
        int numright3 = 0, numwrong3 = 0;
        int numright4 = 0, numwrong4 = 0;
        int numright5 = 0, numwrong5 = 0;
        // проверяем каждое изображение
        for (int i = 0; i < test_samples.size(); ++i)
        {
            lcp.x() = 0;
            ...
            for (int j = 0; j < 31; ++j)
            {
                ...
                cout << test_labels[i](j*2) << " - " << (short)predicted_labels[i](j*2) << " = " << differenceX << endl;
                cout << test_labels[i](j*2+1) << " - " << (short)predicted_labels[i](j*2+1) << " = " << differenceY << endl;
                cin.get();
                ...
                if (differenceX >= 5 || differenceY >= 5) numwrong++;
                else numright++;
                // для области 10х10
                if (differenceX >= 10 || differenceY >= 10) numwrong2++;
                else numright2++;
                // для области 3х3
                if (differenceX >= 3 || differenceY >= 3) numwrong3++;
                else numright3++;
            }
            rcp.x() /= 6;
            rcp.y() /= 6;
            ...
            lcpd.x() /= 6;
            lcpd.y() /= 6;
            differenceX = abs(lcp.x() - lcpd.x());
            differenceY = abs(lcp.x() - lcpd.x());
            // для области 3х3
            if (differenceX >= 3 || differenceY >= 3) numwrong4++;
            else numright4++;
            differenceX = abs(rcp.x() - rcpd.x());
            differenceY = abs(rcp.x() - rcpd.x());
            // для области 3х3
            if (differenceX >= 3 || differenceY >= 3) numwrong5++;
            else numright5++;

        }

        cout << "Number of wrong points in 5x5: " << numwrong << ", right points: " << numright << ". Accuracy: " << fixed << numright/(double)(numright+numwrong) << endl;
        cout << "Number of wrong points in 10x10: " << numwrong2 << ", right points: " << numright2 << ". Accuracy: " << fixed << numright2/(double)(numright2+numwrong2) << endl;
        cout << "Number of wrong points in 3x3: " << numwrong3 << ", right points: " << numright3 << ". Accuracy: " << fixed << numright3/(double)(numright3+numwrong3) << endl;
        cout << "Number of wrong points left eye in 3x3: " << numwrong4 << ", right points: " << numright4 << ". Accuracy: " << fixed << numright4/(double)(numright4+numwrong4) << endl;
        cout << "Number of wrong points right eye in 3x3: " << numwrong5 << ", right points: " << numright5 << ". Accuracy: " << fixed << numright5/(double)(numright5+numwrong5) << endl;
        cout << "Number of wrong points eyes in 3x3: " << numwrong4 + numwrong5 << ", right points: " << numright4 + numright5<< ". Accuracy: " << fixed << (numright4 + numright5)/(double)(numright4+numwrong4+numright5+numwrong5) << endl;
    }
